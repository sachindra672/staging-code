...
export const classroomSocketHandler = (
    ...
    socket.on('produce', async ({ sessionId, transportId, kind, rtpParameters, appData }, callback) => {
        ...
        try {
            ...
            if (existingProducer) {
                ...
                // ✅ SAFE: Stop recording directly (don't emit locally to avoid frontend issues)
                ...
                // Emit to other clients (not locally) to notify them of producer closure
    ...
    socket.on('producer-closed', async ({ sessionId, producerId, peerId, kind, source }) => {
        ...
        try {
            const room = getRoom(sessionId);
            if (!room) {
                console.warn(`⚠️ Room not found for session ${sessionId} in producer-closed`);
                return;
            }

            const peer = room.peers.get(peerId);
            if (!peer) {
                console.warn(`⚠️ Peer ${peerId} not found in room ${sessionId}`);
                return;
            }
            ...
            if (peer.role === "mentor") {
                ...
                if (kind === "audio") {
                    ...
                    // ✅ FIX: Check for screen share FIRST, then camera
                    if (source === "screen" || source === "presentation") {
                        recordingKind = "screen";
                    } else if (source === "camera" || !source) {
                        // Default to video if source is camera or undefined
                        recordingKind = "video";
                    }
                }

                if (recordingKind) {
                    console.log(`🛑 Stopping recording for ${sessionId} (${recordingKind}) - producer closed: ${producerId}, source: ${source}`);
                    await stopRecordingBridge(sessionId, {
                        kind: recordingKind,
                        stoppedAt: Date.now()
                    });
                    console.log(`✅ Stopped recording of ${recordingKind} for session ${sessionId}`);
                } else {
